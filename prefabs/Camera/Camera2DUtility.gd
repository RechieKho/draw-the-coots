...
func get_current_camera() -> Camera2D:
	return get_current_camera_in_viewport(get_viewport())

func get_current_camera_in_viewport(viewport: Node) -> Camera2D:
	...
	for child in viewport.get_children():
		...
		var camera = get_current_camera_in_viewport(child)
		if camera:
			return camera
